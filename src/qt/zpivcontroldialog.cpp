    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zSKN.";
            else if (!mint.isSeedCorrect)
                strReason = "The zSKN seed used to mint this zSKN is not the same as currently hold in the wallet";
            else
