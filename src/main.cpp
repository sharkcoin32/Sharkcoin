...
#if defined(NDEBUG)
#error "SKN cannot be compiled without assertions."
#endif
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const libzerocoin::CoinSpend* spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend->getCoinSerialNumber(), nHeightTx))
        return error("%s : zSKN spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zSKN is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend->HasValidSignature())
                return error("%s: V2 zSKN spend does not have a valid signature\n", __func__);
        ...
        if (spend->getSpendType() != expectedType) {
            return error("%s: trying to spend zSKN without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (pindex->nHeight >= Params().Zerocoin_Block_Public_Spend_Enabled()) {
        ...
        if (v1Serial) {
            return error("%s : zSKN v1 serial spend not spendable, serial %s, tx %s\n", __func__,
    ...
    if (!spend->HasValidSerial(Params().Zerocoin_Params(v1Serial)))  {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
            return error("%s : zSKN spend with serial %s from tx %s is not in valid range\n", __func__,
                     spend->getCoinSerialNumber().GetHex(), tx.GetHash().GetHex());
        else
...

    {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zSKN spend tx %s already in block %d",
            ...
            // Check that zSKN mints (if included) are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    /* block rewards. */
         ...
         if (nHeight >=       1 && nHeight <=       1) {nSubsidy =   30000000 * COIN;}
    else if (nHeight >=       2 && nHeight <=     500) {nSubsidy =       30.00 * COIN;}
    else if (nHeight >=     501 && nHeight <=   50000) {nSubsidy =       27.00 * COIN;}
    else if (nHeight >=   50001 && nHeight <=  100000) {nSubsidy =       22.00 * COIN;}
    else if (nHeight >=  100001 && nHeight <=  200000) {nSubsidy =       18.00 * COIN;}
    else if (nHeight >=  200001 && nHeight <=  300000) {nSubsidy =       16.00 * COIN;}
    else if (nHeight >=  300001 && nHeight <=  400000) {nSubsidy =       14.00 * COIN;}
    else if (nHeight >=  400001 && nHeight <=  500000) {nSubsidy =       15.00 * COIN;}
    else if (nHeight >=  500001 && nHeight <=  600000) {nSubsidy =       12.00 * COIN;}
    else if (nHeight >=  600001 && nHeight <=  700000) {nSubsidy =       8.00 * COIN;}
    else if (nHeight >=  700001 && nHeight <=  800000) {nSubsidy =       7.00 * COIN;}
    else if (nHeight >=  800001 && nHeight <=  900000) {nSubsidy =       6.00 * COIN;}
    else if (nHeight >=  900001 && nHeight <= 1000000) {nSubsidy =       5.00 * COIN;}
    else if (nHeight >= 1000001 && nHeight <= 2500000) {nSubsidy =       4.00 * COIN;}
    else if (nHeight >= 2500001 && nHeight <= 5000000) {nSubsidy =       3.00 * COIN;}
    else if (nHeight >= 5000001 && nHeight <= 7500000) {nSubsidy =       2.00 * COIN;}
    else if (nHeight >= 7500001 && nHeight <=10000000) {nSubsidy =       1.50 * COIN;}
    else if (nHeight >=10000001 && nHeight <=50000000) {nSubsidy =       1.20 * COIN;}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from SKN
...
{
    RenameThread("sharkcoin-scriptch");
...
void RecalculateZPIVSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zSKN supply
...
bool RecalculatePIVSupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating SKN supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating SKN supply..."), percent);
        }
...
{
    // SKN: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZPIVSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    // A one-time event where only the zSKN supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            // Check that zSKN mints are not already known
            if (tx.HasZerocoinMintOutputs()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
    ...
    //Track zSKN money supply in the block index
    if (!UpdateZPIVSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zSKN supply for block=%s height=%d", __func__,
...
void static UpdateTip(CBlockIndex* pindexNew)
{
     ...
     *
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert SKN to zPIV
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zSKN spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zSKN serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Sharkcoin Specific reference to the block with the wrong threshold was used.
        if ((block.nTime == (uint32_t) Params().SharkcoinBadBlockTime()) && (block.nBits == (uint32_t) Params().SharkcoinBadBlocknBits())) {
            // accept SKN block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zSKN inputs.
            if(hasZPIVInputs){
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // SKN: We use certain sporks during IBD, so check to see if they are
