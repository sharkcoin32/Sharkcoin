...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "sharkcoin" is a composite category enabling all SKN-related debug output
            if (ptrCategory->count(std::string("sharkcoin"))) {
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty sharkcoin.conf if it does not exist
        FILE* configFile = fopen(GetConfigFile().string().c_str(), "a");
        if (configFile != NULL)
           {
            std::string strHeader =
                "# Sharkcoin Configuration File!\n"
                "addnode=62.171.178.126\n"
                "addnode=62.171.178.117\n"
                "addnode=62.171.178.115\n"
                "addnode=62.171.170.179\n";
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override sharkcoin.conf
